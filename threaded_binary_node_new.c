...
char * print_threaded_tree( struct threaded_binary_node * self ) {
    ...
    char * template = 
        "<table border='1'>"
            "<tr align='center'>"
                "<td colspan='2'>address : %p<br />value : %p<br />left_link: %p<br />right_link : %p<br />threaded: %c</td>"
            "</tr>"
            "<tr align='center'>"
                "<td>%s</td>"
                "<td>%s</td>"
    ...
    *result_string = '\0';
    sprintf( result_string, template, self, self->data, self->left_link, self->right_link, self->threaded, left_string, right_string );
